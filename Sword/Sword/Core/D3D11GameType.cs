...
namespace SSMT
{
    public class D3D11GameType
    {
        ...
        public int GetElementListTotalStride(List<string> ElementNameList)
        {
            ...
            foreach (string ElementName in ElementNameList)
            {
                ...
                TotalStride += d3D11Element.ByteWidthInt;
            }
            return TotalStride;
        }
        ...
        public void Initialize()
        {
            ...
            foreach (D3D11Element d3D11Element in elements)
            {
                if (SemanticName_Index_Dict.ContainsKey(d3D11Element.SemanticName))
                {
                    ...
                    if (d3D11Element.ByteWidth == "")
                    {
                        d3D11Element.ByteWidth = DBMTFormatUtils.GetByteWidthFromFormat(d3D11Element.Format).ToString();
                    }
            ...
            foreach (D3D11Element d11Element in d3D11Elements01)
            {
                ...
                if (CategoryStrideDict.ContainsKey(d11Element.Category))
                {
                    ...
                    CategoryStrideDict[d11Element.Category] = stride + d11Element.ByteWidthInt;
                }
                else
                {
                    CategoryStrideDict[d11Element.Category] = d11Element.ByteWidthInt;
                }
    ...
    {
        public string PositionExtractSlot { get; set; } = "";
        public string PositionExtractIndex { get; set; } = "";

        public string BlendExtractSlot { get; set; } = "";
        public string BlendExtractIndex { get; set; } = "";

        public D3D11GameType d3d11GameType { get; set; }

        public D3D11GameTypeWrapper(D3D11GameType d3D11GameType)
        {
